...
const EditTamu = () => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    await axios.put(`http://localhost:8080/api/buku-tamu/${id}`, tamu);
    navigate("/");
  };

  const loadData = async () => {
    const result = await axios.get(`http://localhost:8080/api/buku-tamu/${id}`);
    setTamu(result.data);
  }
    ...
    <div className="container">
      <div className="row">
        <div className="col-md-6 offset-md-3 border rounded p-4 mt-2 shadow">
          <h2 className="text-center m-4">Ubah Informasi Tamu</h2>
